...
export async function printDocument(orderData) {
  var printContents = document.getElementById("inv_content")?.innerHTML;
  const popup = window.open("", "_blank", "width=600,height=600");
  popup.document.write(
    `<html><head><title>${orderData?.orderId || "Invoice"}</title>`
  );
  popup.document.write(
    `<link rel="stylesheet" type="text/css" href="${process.env.NEXT_PUBLIC_URL}/css/bootstrap.min.css">`
  );
  popup.document.write(
    `<link rel="stylesheet" type="text/css" href="${process.env.NEXT_PUBLIC_URL}/css/print.css">`
  );
...
export function checkPercentage(number, percentage) {
